...
fast-lint: ruff-check flake8 mypy ###@Code Fast lint code (without pylint)
	...
	$(POETRY_RUN) pytest --dead-fixtures --dup-fixtures
 	# $(POETRY_RUN) safety scan --full-report || echo "Safety scan finished full-report"
...
.PHONY: update
update: db wait-db-ready dump-local docker-build ##@Application Update docker app
	@make docker-stop
...
update-server: ##@Application Update docker app on server
	...
	ssh -p $(PORT) $(USERNAME)@$(HOST) "cd never_forget_bot; git pull && make update; exit;"
	@echo "Server updated"
