...
def validate_json(info: MessageInfo) -> bool | dict:
    ...
    try:
        if type(data['event']) is not str:
            return False
        data['date_of_event'] = (
            datetime.strptime(data['date_of_event'], '%H:%M:%S %d.%m.%Y')
            .replace(tzinfo=timezone(timedelta(hours=info.user_tz)))
            .isoformat()
        )
        data['date_of_notify'] = (
            datetime.strptime(data['date_of_notify'], '%H:%M:%S %d.%m.%Y')
            .replace(tzinfo=timezone(timedelta(hours=info.user_tz)))
            .isoformat()
        )
...
async def add_event_process(data: MessageInfo) -> Notifydata:
    async with SessionManager().create_async_session(expire_on_commit=False) as session:
        ...
        event = await add_event(
            session, data.gpt_json['type_of_event'], data.gpt_json['event'], date_of_event, data.user_id
        )
...
class ProcessMessageWorkflow:
    ...
    async def run(self, data: MessageInfo) -> None:
        ...
        workflow.logger.info('GPT JSON: %s\n', data)
        result: Notifydata = await workflow.execute_activity(
