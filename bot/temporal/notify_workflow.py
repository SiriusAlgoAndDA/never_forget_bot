...
from bot.utils.user_utils import get_user
...
async def send_notify(data: Notifydata) -> str | Notifydata:
    ...
    async with SessionManager().create_async_session(expire_on_commit=False) as session:
        ...
        return Notifydata(notify.id, msg.date)
...
async def update_db(data: Notifydata) -> str | Notifydata:
    async with SessionManager().create_async_session(expire_on_commit=False) as session:
        ...
        notify = await update_sent_ts(session, data.notify_id, data.sent_ts)
        if notify is None:
            return 'Notification is None'
        event = await get_event(session, notify.event_id)
        if event is None:
        ...
        if user is None:
            return 'User is None'
        new_notify_ts = data.sent_ts + event.reschedule_timedelta
...
class NotificationWorkflow:
    ...
    async def run(self, data: Notifydata) -> None:
        # Выполняем активити
        result = await workflow.execute_activity(send_notify, data)
        workflow.logger.info('Sent message: %s\nNotify_id: %s', result, data.notify_id)
        if result is str:
            return
        result: str | Notifydata = await workflow.execute_activity(update_db, result)
        workflow.logger.info('Update db\nNotify_old_id: %s \nNotify New ID', data.notify_id, result.notify_id)
        if result is str:
            return
        result = await add_new_workflow(result)
