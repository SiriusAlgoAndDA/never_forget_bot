...
class Notification(BaseModel):
    ...
    event_id: orm.Mapped[str] = sa.Column(
        'event_id',
        sa.UUID(as_uuid=True),
        sa.ForeignKey('event.id'),  # Ссылка на таблицу `event`
        nullable=False,
        index=True,
        doc="ID event'а"
    )
